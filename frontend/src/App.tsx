...
export default function App(){
 ...
 return <div className="app"><Sidebar active={active} setActive={setActive}/><main><div className="topbar"><div className="filterSummary"><b>Filtros globais</b><span>{count ? `${count} filtros ativos` : "Sem filtros ativos"}</span></div><div className="topFilterChips">{entries.map(entry=><button className={`filterChip ${entry.limited?"limited":""}`} key={entry.key} title={`Remover filtro ${entry.label}: ${entry.value}`} aria-label={`Remover filtro ${entry.label}: ${entry.value}`} onClick={()=>removeGlobalFilter(entry.key)}>{entry.label}: {entry.value}<span aria-hidden="true">x</span></button>)}</div><div className="topActions"><span className={`apiStatus ${apiStatus}`} role="status">{apiStatusLabel(apiStatus)}</span><button title="Limpar filtros globais" aria-label="Limpar filtros globais" disabled={!count} onClick={clearGlobalFilters}>↺</button><button title={fullscreen?"Sair da tela cheia":"Tela cheia"} aria-label={fullscreen?"Sair da tela cheia":"Tela cheia"} onClick={toggleFullscreen}>⛶</button><div className="user"><div className="avatar">A</div><div><b>Analista</b><small>MVP</small></div></div></div></div><div className="content"><Suspense fallback={<div className="emptyState">Carregando módulo...</div>}>{page}</Suspense></div></main></div>
}
