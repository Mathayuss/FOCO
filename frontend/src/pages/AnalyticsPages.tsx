...
function toggleText(current:string, next:string){
 return current === next ? "" : next
}

function togglePeriod(current:string, next:string){
...
function useGlobalParams(globalFilters:GlobalFilters){
 const {source,period,type,municipality,unit,subtype,shift}=globalFilters
 return useMemo(()=>toAnalyticsParams(globalFilters),[source,period,type,municipality,unit,subtype,shift])
}
...
export function EvolutionPage({globalFilters,setGlobalFilters}:DashboardProps){
 ...
 const series:any[]=[{name:comparisonSummary?.current_label || "SEJUSP",type:"line",smooth:true,symbolSize:(_:unknown,p:any)=>highlightPeriod&&selectedMonths.has(state.monthly[p.dataIndex]?.mes)?10:7,data:state.monthly.map(item=>item.total),lineStyle:{width:3,color:"#d83135"},itemStyle:{color:"#ffcc29"}}]
 if(hasComparison) series.unshift({name:comparisonSummary?.baseline_label || "Período anterior",type:"line",smooth:true,symbolSize:5,data:state.comparison.map(item=>item.baseline),lineStyle:{width:2,color:"#58a6ff",type:"dashed"},itemStyle:{color:"#58a6ff"}})
 ...
 return <><PageHead area="ANÁLISE / EVOLUÇÃO" title="Evolução" caption="Volume mensal e comparação disponível." badge="Série temporal"/><FilterBar filters={state.filters} globalFilters={globalFilters} setGlobalFilters={setGlobalFilters}/><Warning items={state.overview?.unavailable_filters}/><LoadState state={state}><div className="kpiGrid compact"><KpiCard label="Total" value={fmt(state.overview?.total || 0)} meta="período selecionado" tone="red"/><KpiCard label="Média diária" value={dec(state.overview?.average_per_day || 0)} meta="dias reais do período"/><KpiCard label="Variação" value={state.overview?.delta_pct==null?"sem base":`${state.overview.delta_pct>0?"+":""}${dec(state.overview.delta_pct)}%`} meta={state.overview?.comparison?.baseline_label || state.overview?.comparison?.reason || "período anterior"} tone="gold"/><KpiCard label="Base comparada" value={comparisonSummary?.baseline_total==null?"sem base":fmt(comparisonSummary.baseline_total)} meta={comparisonSummary?.baseline_label || comparisonSummary?.reason || "comparativo histórico"} tone="neutral"/></div><Panel title="Evolução mensal" sub={hasComparison?`${comparisonSummary?.current_label} comparado com ${comparisonSummary?.baseline_label}`:"Série filtrada pela API"}><ReactECharts option={option} onEvents={monthEvents} style={{height:390}}/></Panel></LoadState></>
}
...
export function TypificationPage({globalFilters,setGlobalFilters}:DashboardProps){
 ...
 const typeEvents={click:(p:any)=>{ if(p?.name) setGlobalFilters({type:toggleText(globalFilters.type,String(p.name))}) }}
 const option={tooltip:{trigger:"item"},grid:{left:150,right:24,top:8,bottom:26},xAxis:{type:"value",...axis},yAxis:{type:"category",inverse:true,data:items.slice(0,12).map(item=>item.nome),...axis,axisLabel:{color:"#aeb4bd",fontSize:10,width:132,overflow:"truncate"}},series:[{type:"bar",data:items.slice(0,12).map(item=>item.total),barWidth:12,itemStyle:{borderRadius:6,color:(p:any)=>items[p.dataIndex]?.nome===globalFilters.type?"#ffcc29":"#d83135"}}]}
 return <><PageHead area="ANÁLISE / TIPIFICAÇÃO" title="Tipificação" caption="Distribuição das principais naturezas de ocorrência." badge="Categorias"/><FilterBar filters={state.filters} globalFilters={globalFilters} setGlobalFilters={setGlobalFilters}/><Warning items={state.types?.unavailable_filters}/><LoadState state={state}><div className="grid twoOne"><Panel title="Ranking de tipificações" sub="Totais do recorte disponível"><ReactECharts option={option} onEvents={typeEvents} style={{height:420}}/></Panel><Panel title="Tabela" sub="Clique em uma linha para filtrar"><Table items={items} onSelect={item=>setGlobalFilters({type:toggleText(globalFilters.type,item.nome)})}/></Panel></div></LoadState></>
}
...
export function TemporalPage({globalFilters,setGlobalFilters}:DashboardProps){
 ...
 const hourEvents={click:(p:any)=>{ const index=Number(p?.dataIndex); if(globalFilters.source==="sejusp" && Number.isInteger(index)){ const next=shiftForHour(index); setGlobalFilters({shift:toggleText(globalFilters.shift,next)}) } }}
 const hourOption={tooltip:{trigger:"axis"},grid:{left:42,right:16,top:18,bottom:34},xAxis:{type:"category",data:hours.map((_,i)=>String(i).padStart(2,"0")),...axis},yAxis:{type:"value",...axis},series:[{type:"bar",data:hours,barWidth:"62%",itemStyle:{color:(p:any)=>globalFilters.shift&&shiftForHour(p.dataIndex)===globalFilters.shift?"#ffcc29":p.dataIndex===peakHour?"#ffcc29":p.dataIndex>=7&&p.dataIndex<=18?"#d83135":"#343a44",borderRadius:[3,3,0,0]}}]}
 const shiftEvents={click:(p:any)=>{ if(p?.name) setGlobalFilters({shift:toggleText(globalFilters.shift,String(p.name))}) }}
 const shiftOption={tooltip:{trigger:"item"},grid:{left:92,right:18,top:12,bottom:22},xAxis:{type:"value",...axis},yAxis:{type:"category",inverse:true,data:shifts.map(item=>item.nome),...axis},series:[{type:"bar",data:shifts.map(item=>item.total),barWidth:14,itemStyle:{borderRadius:6,color:(p:any)=>shifts[p.dataIndex]?.nome===globalFilters.shift?"#ffcc29":"#58a6ff"}}]}
 return <><PageHead area="ANÁLISE / TEMPORAL" title="Temporal" caption="Perfil horário e distribuição por turno." badge="Tempo"/><FilterBar filters={state.filters} globalFilters={globalFilters} setGlobalFilters={setGlobalFilters}><select className={globalFilters.shift&&state.filters?.limited_dimensions?.includes("shift")?"filterLimited":""} value={globalFilters.shift} onChange={e=>setGlobalFilters({shift:e.target.value})}><option value="">Todos os turnos</option>{(state.filters?.shifts || []).map(item=><option key={item} value={item}>{item}</option>)}</select></FilterBar><Warning items={[...(state.hours?.unavailable_filters || []),...(state.shifts?.unavailable_filters || [])]}/><LoadState state={state}><div className="kpiGrid compact"><KpiCard label="Pico horário" value={`${String(peakHour).padStart(2,"0")}h`} meta={`${fmt(hours[peakHour] || 0)} ocorrências`} tone="gold"/><KpiCard label="Turnos" value={String(shifts.length)} meta={globalFilters.source==="sejusp"?"recorte importado":"consolidado histórico"}/><KpiCard label="Fonte" value={globalFilters.source==="sejusp"?"SEJUSP":"Histórico"} meta={globalFilters.source==="sejusp"?"com filtros cruzados":"sem cruzamento por período/tipo"} tone="neutral"/></div><div className="grid twoOne"><Panel title="Ocorrências por hora" sub="Perfil consolidado"><ReactECharts option={hourOption} onEvents={hourEvents} style={{height:380}}/></Panel><Panel title="Turnos" sub={globalFilters.shift?"Turno selecionado":"Distribuição consolidada"}><ReactECharts option={shiftOption} onEvents={shiftEvents} style={{height:380}}/></Panel></div></LoadState></>
}
...
export function UnitsPage({globalFilters,setGlobalFilters}:DashboardProps){
 ...
 const unitEvents={click:(p:any)=>{ if(p?.name) setGlobalFilters({unit:toggleText(globalFilters.unit,String(p.name))}) }}
 const option={tooltip:{trigger:"item"},grid:{left:135,right:18,top:8,bottom:20},xAxis:{type:"value",...axis},yAxis:{type:"category",inverse:true,data:items.map(item=>item.nome),...axis,axisLabel:{color:"#aeb4bd",fontSize:10,width:118,overflow:"truncate"}},series:[{type:"bar",data:items.map(item=>item.total),barWidth:12,itemStyle:{borderRadius:6,color:(p:any)=>items[p.dataIndex]?.nome===globalFilters.unit?"#ffcc29":"#58a6ff"}}]}
 return <><PageHead area="OPERAÇÃO / UNIDADES" title="Unidades" caption="Distribuição consolidada por unidade operacional." badge="Consolidado"/><FilterBar filters={state.filters} globalFilters={globalFilters} setGlobalFilters={setGlobalFilters}><select className={globalFilters.unit&&state.filters?.limited_dimensions?.includes("unit")?"filterLimited":""} value={globalFilters.unit} onChange={e=>setGlobalFilters({unit:e.target.value})}><option value="">Todas as unidades</option>{(state.filters?.units || []).map(item=><option key={item} value={item}>{item}</option>)}</select></FilterBar><Warning items={state.units?.unavailable_filters}/><LoadState state={state}><div className="grid twoOne"><Panel title="Ranking de unidades" sub={globalFilters.unit?"Unidade selecionada":"Ocorrências consolidadas"}><ReactECharts option={option} onEvents={unitEvents} style={{height:420}}/></Panel><Panel title="Tabela" sub={`${fmt(total)} ocorrências exibidas`}><Table items={items} label="Unidade" onSelect={item=>setGlobalFilters({unit:toggleText(globalFilters.unit,item.nome)})}/></Panel></div></LoadState></>
}
