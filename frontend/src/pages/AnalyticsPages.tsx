...
export function EvolutionPage({globalFilters,setGlobalFilters}:DashboardProps){
 ...
 const series:any[]=[{name:globalFilters.source==="sejusp"?"SEJUSP 2025":"2026",type:"line",smooth:true,symbolSize:(_:unknown,p:any)=>highlightPeriod&&selectedMonths.has(state.monthly[p.dataIndex]?.mes)?10:7,data:state.monthly.map(item=>item.total),lineStyle:{width:3,color:"#d83135"},itemStyle:{color:"#ffcc29"}}]
...
export function TemporalPage({globalFilters,setGlobalFilters}:DashboardProps){
 ...
 const hourOption={tooltip:{trigger:"axis"},grid:{left:42,right:16,top:18,bottom:34},xAxis:{type:"category",data:hours.map((_,i)=>String(i).padStart(2,"0")),...axis},yAxis:{type:"value",...axis},series:[{type:"bar",data:hours,barWidth:"62%",itemStyle:{color:(p:any)=>p.dataIndex===peakHour?"#ffcc29":p.dataIndex>=7&&p.dataIndex<=18?"#d83135":"#343a44",borderRadius:[3,3,0,0]}}]}
