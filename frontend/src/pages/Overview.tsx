...
export default function Overview({globalFilters,setGlobalFilters,clearGlobalFilters}:OverviewProps){
 ...
 const monthlyOption=useMemo(()=>{
  ...
  const series:any[]=[{name:source==="sejusp"?"SEJUSP 2025":"2026",type:"line",data:months.map(x=>x.total),smooth:true,symbolSize:(_:unknown,p:any)=>highlightPeriod&&selectedMonths.has(months[p.dataIndex]?.mes)?10:7,lineStyle:{width:3,color:"#d83135"},itemStyle:{color:"#ffcc29"},areaStyle:{color:{type:"linear",x:0,y:0,x2:0,y2:1,colorStops:[{offset:0,color:"rgba(216,49,53,.32)"},{offset:1,color:"rgba(216,49,53,.02)"}]}}}]
 ...
 return <>
  ...
  <div className="scopeNote">{source==="sejusp"?<><b>SEJUSP importado</b> alimenta os painéis com linhas reais importadas e filtros cruzados por período, tipo, município, unidade, subtipo e turno.</>:<><b>Histórico consolidado</b> alimenta volume, evolução e tipificação filtráveis por período/tipo. <b>Território, hora e unidade</b> aguardam agregações cruzadas para refletir todos os filtros.</>}</div>
