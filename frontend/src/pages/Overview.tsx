...
import { activeFilterCount, toAnalyticsParams, type GlobalFilters, type SetGlobalFilters } from "../filterState"
import type { AvailableFilters, MonthlyComparison, MonthlyItem, NamedMetric, Overview as OverviewType, Sla } from "../types"
...
export default function Overview({globalFilters,setGlobalFilters,clearGlobalFilters}:OverviewProps){
 ...
 return <>
  ...
  <div className="activeFilters"><i>Período: {ov?.applied_filters?.period || periodOptions.find(x=>x.key===period)?.label}</i>{typeFilter&&<i>Tipo: {typeFilter}</i>}{municipality&&<i className="limited">Município solicitado: {municipality}</i>}{unit&&<i className="limited">Unidade solicitada: {unit}</i>}{shift&&<i className="limited">Turno solicitado: {shift}</i>}<i>Fonte: histórico consolidado</i>{hasComparison&&<i>Comparativo: 2025 x 2026</i>}{limited.length>0&&<i className="limited">Limitados: {limited.join(", ")}</i>}</div>
